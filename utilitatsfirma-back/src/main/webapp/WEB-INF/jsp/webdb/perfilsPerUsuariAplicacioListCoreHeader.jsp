...
<un:useConstants var="PerfilsPerUsuariAplicacioFields" className="es.caib.utilitatsfirma.model.fields.PerfilsPerUsuariAplicacioFields"/>
        ...
        <th>${suf:getSortIcons(__theFilterForm,PerfilsPerUsuariAplicacioFields.PERFILSPERUSRAPPID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,PerfilsPerUsuariAplicacioFields.PERFILDEFIRMAID)}">
        <th>${suf:getSortIcons(__theFilterForm,PerfilsPerUsuariAplicacioFields.PERFILDEFIRMAID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,PerfilsPerUsuariAplicacioFields.USUARIAPLICACIOID)}">
        <th>${suf:getSortIcons(__theFilterForm,PerfilsPerUsuariAplicacioFields.USUARIAPLICACIOID)}</th>
        </c:if>
