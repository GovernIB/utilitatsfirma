...
public class GenericMessageBodyReader implements MessageBodyReader<IMessageBodyReader> {
    ...
    @Override
    public IMessageBodyReader readFrom(Class<IMessageBodyReader> type, Type genericType, Annotation[] annotations,
        ...
        try {
            ...
            try {
                ...
                throw new IOException("[" + e.getClass().getName() + "] Error instanciant objecte de tipus "
                ...
                if (File.class.equals(fieldType) || byte[].class.equals(fieldType)) {
                    ...
                    if (schema == null || !schema.hidden()) {
                        throw new InternalServerErrorException(
                                "@Schema(hidden=true) required on field " + field.getName());
                    }

                    String name = field.getName();

                    if (!name.endsWith("PartInfo")) {
                        throw new InternalServerErrorException("Field name " + name
                                + " should end with 'FileInfo'. This class is reserved for attach additional information of a file field");
                    }
            ...
            for (InputPart part : multipartInput.getParts()) {
                ...
                */
                ...
                switch (fi.getType()) {
                    case 1: // File
                    {
                        MultipartFileInfo fileInfo = MultipartUtils.getMultipartFileInfo(className, formParamName,
                                part);
                        ...
                        try {
                            ...
                            throw new InternalServerErrorException(
                                    "Error setting File field " + fi.getField().getName() + ": " + e.getMessage(), e);
                        }
                        ...
                        if (formPartInfoField != null) {
                            ...
                            try {
                                ...
                                throw new InternalServerErrorException("Error setting FormFileInfo field "
                                        + formPartInfoField.getName() + ": " + e.getMessage(), e);
                            }
                        }
                    }
                    break;
                    case 2: // byte[]
                        try {
                            ...
                            MultipartByteArrayInfo byteArrayInfo = MultipartUtils.getMultipartByteArrayInfo(className,
                                    formParamName, part);
                            ...
                            Field formPartInfoField = partInfoFieldsByFieldName
                                    .get(fi.getField().getName() + "PartInfo");
                            if (formPartInfoField != null) {
                                formPartInfoField.setAccessible(true);
                                try {
                    ...
                    case 4: // JSON Object
                        try {
                            ...
                            log.info("\n\nProcessing JSON Object field " + fi.getField().getName() 
                            + " from multipart part with formParamName: " + formParamName + "\n\n");
                            */
            ...
            throw new InternalServerErrorException("Error No controlat en GenericMessageBodyReader: " + e.getMessage(),
                    e);
        }
    }

    public static String readFormParamValue(Field field) {
