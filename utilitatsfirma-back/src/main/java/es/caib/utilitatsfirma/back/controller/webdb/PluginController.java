...
import es.caib.utilitatsfirma.back.validator.webdb.PluginWebValidator;
 ...
 * @author GenApp
 */
@MenuOption(labelCode="plugin.plugin.plural", order=50, group=Tab.MENU_WEBDB)
@Controller
...
public class PluginController
    extends es.caib.utilitatsfirma.back.controller.UtilitatsFirmaBaseController<Plugin, java.lang.Long> implements PluginFields {

  @EJB(mappedName = es.caib.utilitatsfirma.ejb.IdiomaService.JNDI_NAME)
  protected es.caib.utilitatsfirma.ejb.IdiomaService idiomaEjb;

  @EJB(mappedName = es.caib.utilitatsfirma.ejb.PluginService.JNDI_NAME)
  ...
  public ModelAndView crearPluginGet(HttpServletRequest request,
    ...
    if (pluginForm.getPlugin().getNom() == null){
      es.caib.utilitatsfirma.persistence.TraduccioJPA trad = new es.caib.utilitatsfirma.persistence.TraduccioJPA();
      for (es.caib.utilitatsfirma.model.entity.Idioma idioma : pluginForm.getIdiomesTraduccio()) {
        trad.addTraduccio(idioma.getIdiomaID(), new es.caib.utilitatsfirma.persistence.TraduccioMapJPA());
      }
  ...
  public List<es.caib.utilitatsfirma.model.entity.Idioma> getIdiomesSuportats() throws I18NException {
    List<es.caib.utilitatsfirma.model.entity.Idioma> idiomes = idiomaEjb.select(es.caib.utilitatsfirma.model.fields.IdiomaFields.SUPORTAT.equal(true));
    return idiomes;
  }
...
public java.lang.Long stringToPK(String value) {
 ...
 exceptionMsg };
  ...
  public List<StringKeyValue> getReferenceListForTipus(HttpServletRequest request,
    ...
    __tmp.add(new StringKeyValue("0" , "Validaci\u00F3"));
    __tmp.add(new StringKeyValue("1" , "Firma Servidor"));
    return __tmp;
  }
