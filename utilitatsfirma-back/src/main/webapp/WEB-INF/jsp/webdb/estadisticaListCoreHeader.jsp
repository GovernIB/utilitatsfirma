...
<un:useConstants var="EstadisticaFields" className="es.caib.utilitatsfirma.model.fields.EstadisticaFields"/>
        ...
        <th>${suf:getSortIcons(__theFilterForm,EstadisticaFields.ESTADISTICAID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,EstadisticaFields.DATA)}">
        <th>${suf:getSortIcons(__theFilterForm,EstadisticaFields.DATA)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,EstadisticaFields.TIPUS)}">
        <th>${suf:getSortIcons(__theFilterForm,EstadisticaFields.TIPUS)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,EstadisticaFields.VALOR)}">
        <th>${suf:getSortIcons(__theFilterForm,EstadisticaFields.VALOR)}</th>
        </c:if>
