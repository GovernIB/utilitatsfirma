...
import es.caib.utilitatsfirma.hibernate.HibernateFileUtil;
import es.caib.utilitatsfirma.logic.utils.I18NLogicUtils;
import es.caib.utilitatsfirma.logic.utils.LogicUtils;
    ...
    public void init(ServletConfig config) throws ServletException {
            ...
            public void run() {
                try {
                    MenuOptionManager.setDiscoverMenuOptionAnnotations(
                            new DiscoverMenuOptionAnnotations(Constants.UTILITATSFIRMA_PROPERTY_BASE + "back.controller"));
                } catch (Throwable th) {
        ...
        try {
            ...
            if (fd == null) {
                throw new Exception("No s'ha definit la propietat de la ubicació dels fitxers ("
                        +  "es.caib.utilitatsfirma.filesdirectory)") ;
            }
            if (!fd.exists()) {
        ...
        try {
            ...
            String[] basenames = { "missatges", // /WEB-INF/classes/
                    "logicmissatges", "genapp", "utilitatsfirma_genapp" };
            ...
            if (plugins == null || plugins.size() == 0) {
                ...
                for (Class<? extends IExportDataPlugin> class1 : plugins) {
                    ...
                    if (edp == null) {
                        ...
                        DataExporterManager.addDataExporter(new es.caib.utilitatsfirma.back.utils.UtilitatsFirmaDataExporter(edp));
                    }
                }
            }
        ...
        try {
            log.info("UtilitatsFirma Version: " + ver);
        } catch (Throwable e) {
            log.info("UtilitatsFirma Version: " + ver);
        }
