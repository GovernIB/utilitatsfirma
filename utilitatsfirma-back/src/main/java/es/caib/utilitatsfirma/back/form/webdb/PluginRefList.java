...
import es.caib.utilitatsfirma.ejb.TraduccioService;
...
public class PluginRefList extends RefListBase implements PluginFields {
    ...
    public List<StringKeyValue> getReferenceList(Field<?> keyField, Where where, OrderBy ... orderBy) throws I18NException {
        ...
        _w1 = es.caib.utilitatsfirma.model.fields.TraduccioFields.TRADUCCIOID.in(pluginEjb.getSubQuery(_transSelect, where));
        List<es.caib.utilitatsfirma.model.entity.Traduccio> traduccions = traduccioEjb.select(_w1);
        ...
        for (es.caib.utilitatsfirma.model.entity.Traduccio traduccio : traduccions) {
            es.caib.utilitatsfirma.persistence.TraduccioJPA traduccioJPA = (es.caib.utilitatsfirma.persistence.TraduccioJPA) traduccio;
            String key = keysMap.get(String.valueOf(traduccioJPA.getTraduccioID()));
            es.caib.utilitatsfirma.persistence.TraduccioMapJPA _tm = traduccioJPA.getTraduccio(_lang);
            String value;
            if (_tm == null) {
