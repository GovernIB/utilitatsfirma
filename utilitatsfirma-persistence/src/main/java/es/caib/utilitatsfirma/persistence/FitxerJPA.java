...
@Table(name = "suf_fitxer" , indexes = { 
        @Index(name="suf_fitxer_pk_i", columnList = "fitxerid")})
@SequenceGenerator(name="FITXER_SEQ", sequenceName="suf_fitxer_seq", allocationSize=1, initialValue=1000)
...
// EXP  Field:detachedid | Table: suf_validacio | Type: 0  

    @OneToMany(fetch = FetchType.LAZY, mappedBy = "detachedID")
    private Set<ValidacioJPA> validacio_detachedids = new HashSet<ValidacioJPA>(0);
    public  Set<ValidacioJPA> getValidacio_detachedids() {
    return this.validacio_detachedids;
  }

    public void setValidacio_detachedids(Set<ValidacioJPA> validacio_detachedids) {
      this.validacio_detachedids = validacio_detachedids;
    }


// EXP  Field:signaturaid | Table: suf_validacio | Type: 0  

    @OneToMany(fetch = FetchType.LAZY, mappedBy = "signaturaID")
    private Set<ValidacioJPA> validacio_signaturaids = new HashSet<ValidacioJPA>(0);
    public  Set<ValidacioJPA> getValidacio_signaturaids() {
    return this.validacio_signaturaids;
  }

    public void setValidacio_signaturaids(Set<ValidacioJPA> validacio_signaturaids) {
      this.validacio_signaturaids = validacio_signaturaids;
    }
  ...
  void postLoad() {
    if (threadLocal.get() != null) {
      this.encryptedFileID = es.caib.utilitatsfirma.hibernate.HibernateFileUtil.encryptFileID(fitxerID);
    }
  }
 ...
 // ---------------  STATIC METHODS ------------------
  ...
  static FitxerJPA copyJPA(FitxerJPA __jpa,
    ...
    if(!"ValidacioJPA".equals(origenJPA) 
       && ( !org.fundaciobit.genapp.common.utils.Utils.isEmpty(__jpa.validacio_detachedids) || org.hibernate.Hibernate.isInitialized(__jpa.getValidacio_detachedids())) ) {
      __tmp.setValidacio_detachedids(ValidacioJPA.copyJPA(__jpa.getValidacio_detachedids(), __alreadyCopied,"FitxerJPA"));
    }
    if(!"ValidacioJPA".equals(origenJPA) 
       && ( !org.fundaciobit.genapp.common.utils.Utils.isEmpty(__jpa.validacio_signaturaids) || org.hibernate.Hibernate.isInitialized(__jpa.getValidacio_signaturaids())) ) {
      __tmp.setValidacio_signaturaids(ValidacioJPA.copyJPA(__jpa.getValidacio_signaturaids(), __alreadyCopied,"FitxerJPA"));
    }
