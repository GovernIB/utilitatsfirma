...
public class ValidationsCAdES {
    ...
    public static final Logger log = Logger.getLogger(ValidationsCAdES.class);

    public static byte[] getOriginalDocumentOfCadesAttachedSignature(InputStream eSignature) throws I18NException {
        ...
        try {
            ...
            throw new I18NException("genapp.comodi",
                    "Error desconegut a l'hora d´extreure el document Original d'una Firma"
                            + " CAdEs-Attached certificats d'una firma CAdES " + e.getMessage());
        }
     ...
     *           in error case.
     */
    public static X509Certificate[] getCertificatesOfCadesSignature(InputStream eSignature, byte[] document)
        ...
        try {
            ...
            return getCertificatesOfCadesSignature(signedData.getSignerInfos(), signedData.getCertificates());

        } catch (Exception e) {

            // XYZ ZZZ TRA
            throw new I18NException("genapp.comodi",
                    "Error desconegut a l'hora d´extreure els certificats d'una firma CAdES " + e.getMessage());
        }
    }
     ...
     *           in error case.
     */
    private static X509Certificate[] getCertificatesOfCadesSignature(SignerInformationStore signerInfos,
            Store<?> certificates) throws I18NException {
        ...
        try {
            for (Iterator<?> iterator = signerInfos.getSigners().iterator(); iterator.hasNext();) {
                ...
                if (x509certificateholder == null) {
                    ...
                    throw new Exception("El certificat per l'ID " + signerInformation.getSID() + " es NULL");
                }
            ...
            String message = "Error desconegut a l'hora d´extreure els certificats d'una firma CAdES "
                    + th.getMessage();

            log.error(message, th);

            throw new I18NException(th, "genapp.comodi", message);
        }
     ...
     */
    private static X509CertificateHolder getCertificateBySignerId(Store<?> certificates, SignerId signerId) {
