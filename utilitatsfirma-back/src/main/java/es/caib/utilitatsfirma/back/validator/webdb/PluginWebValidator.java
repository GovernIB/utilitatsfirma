...
import es.caib.utilitatsfirma.persistence.validator.PluginValidator;
...
public class PluginWebValidator extends AbstractWebValidator<PluginForm, Plugin>
  ...
  @javax.ejb.EJB(mappedName = es.caib.utilitatsfirma.ejb.TraduccioService.JNDI_NAME)
...
_ignoreFields.add(DESCRIPCIOCURTAID);
  ...
  {
      es.caib.utilitatsfirma.persistence.PluginJPA __jpa;
      __jpa = (es.caib.utilitatsfirma.persistence.PluginJPA)__bean;
    {
      // IF CAMP ES NOT NULL verificar que totes les traduccions son not null
      es.caib.utilitatsfirma.persistence.TraduccioJPA tradJPA = __jpa.getNom();
      if (tradJPA != null) {
        // TODO ERROR
        java.util.Map<String,es.caib.utilitatsfirma.persistence.TraduccioMapJPA> _trad = tradJPA.getTraduccions();
        int countNotNull = 0;
        for (String _idioma : _trad.keySet()) {
          es.caib.utilitatsfirma.persistence.TraduccioMapJPA _map = _trad.get(_idioma);
    ...
    {
      // IF CAMP ES NOT NULL verificar que totes les traduccions son not null
      es.caib.utilitatsfirma.persistence.TraduccioJPA tradJPA = __jpa.getDescripcioCurta();
      if (tradJPA != null) {
        // TODO ERROR
        java.util.Map<String,es.caib.utilitatsfirma.persistence.TraduccioMapJPA> _trad = tradJPA.getTraduccions();
        int countNotNull = 0;
